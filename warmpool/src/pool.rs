...
impl TemplatePoolBuilder {
    ...
    /// every test database. The predicate also feeds the fingerprint.
    /// An excluded migration is never applied to the template, but it *is* applied
    /// to any test database you create with [`TemplatePool::create_test_database`].
    /// excluded migration doesn't affect the template name at all.
    ...
    pub async fn build(self) -> Result<TemplatePool> {
        ...
        Ok(TemplatePool {
            ...
            template_name: Arc::new(OnceCell::new()),
        })
    }
}
...
pub struct TemplatePool {
    ...
    template_prefix: String,
    fingerprint_salt: Option<String>,
    template_name: Arc<OnceCell<String>>,
}
...
impl TemplatePool {
    ...
    pub async fn create_test_database(&self) -> Result<TestDatabase> {
        let template_name = self.ensure_template().await?;
        ...
        maintenance
            .execute(
                format!(r#"CREATE DATABASE "{db_name}" WITH TEMPLATE "{template_name}";"#)
                    .as_str(),
            )
            .await
    ...
    async fn ensure_template(&self) -> Result<String> {
...
async fn build_or_reuse_template(
    ...
    let build_result =
        build_template_if_missing(&mut maintenance, connect_options, &template_name, migrations)
            .await;
...
async fn build_template_if_missing(
    ...
    let template_pool = PgPool::connect_with(template_opts).await.map_err(|source| {
        Error::TemplatePoolConnect {
            name: template_name.to_string(),
            source,
        }
    })?;
...
async fn run_migrations(pool: &PgPool, template_name: &str, migrations: &[Migration]) -> Result<()> {
    pool.execute(
        r#"
